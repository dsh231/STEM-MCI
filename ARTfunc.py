## updated 1 Jan 2025 (Budhitama Subagdja) -- fast matrix calculation for match function 
##-------------------------------------------
...
#matchFuzzyQuick: Fuzzy match function of weight vector wck with vector xck for all codes
#return the match values in a field for all codes -- Jan 2025
def matchFuzzyQuick(xck,wck):	
	return np.sum(np.minimum(xck,list(wck)),axis=1) * ((np.sum(xck))**-1.0)


#matchFuzzyQuick: Fuzzy match function of weight vector wc with vector xc for all fields and codes
#return the match values for all fields and codes -- Jan 2025
def matchAllFuzzyQuick(xc,wc):	
	return [list(matchFuzzyQuick(xc[k],wc[k])) for k in range(len(xc))] #returning list of match values for every field for all codes

#codeResonanceCheck: Check if code j is in resonance condition, given matching values of all fields and codes, code index j, and vigilance parameters
#return True (if resonance) or False (if not in resonance) -- Jan 2025
def codeResonanceCheckQuick(mlist, j, rhos):
	mclist = np.array(mlist, dtype=object)
	mcjlist = np.transpose(mclist)[j]
	crhos = np.array(rhos, dtype=object)
	return all(crhos <= mcjlist)

def singleCodeResonanceQuick(mckj, rhos):
	#mcjlist = np.array(mckj, dtype=object)
	#crhos = np.array(rhos, dtype=object)
	answer = all([rhos[i] <= mckj[i] for i in range(len(rhos))])
	print(f'all {rhos} <= {mckj} {answer}')
	return answer

#fieldTResonanceCheckQuick: Check if code j is in resonance condition, given matching values of all fields and codes, code index j, and vigilance parameters
#return True (if resonance) or False (if not in resonance) -- Jan 2025
def fieldTResonanceCheckQuick(mck, j, rhos, pcrit, anye=False):
	mckj = np.array(mck[j], dtype=object)
	crhos = np.array(rhos, dtype=object)
	pcrits = np.array(pcrit, dtype=object)
	if anye:
		return any([crhos[i] <= mckj[i] for i in list(np.where(pcrits >= 1)[0])])
	return all(crhos <= mckj)




#resonanceCheckQuick: Check the resonance conditions for all codes, given matching values of all fields and codes, and vigilance parameters
#return list of resonance conditions (TRUE-FALSE) for all codes -- Jan 2025
def resonanceCheckQuick(mlist, rhos, anye=False):
	mclist = np.array(mlist, dtype=object)
	crhos = np.array(rhos, dtype=object)
	mclistT = np.transpose(mclist)
	if anye:
		return (crhos <= mclistT).any(axis=1)
	return (crhos <= mclistT).all(axis=1)

def dualResonanceCheckQuick(mlist, mlistany, rhos):
	mclist = np.array(mlist, dtype=object)
	mclistany = np.array(mlist, dtype=object)
	crhos = np.array(rhos, dtype=object)
	mclistT = np.transpose(mclist)
	mclistanyT = np.transpose(mclistany)
	return (crhos <= mclistT).all(axis=1), (crhos <= mclistanyT).any(axis=1) 

def contrastiveResonanceCheckQuick(mlist, rhos):
	mclist = np.array(mlist, dtype=object)
	crhos = np.array(rhos, dtype=object)
	mclistT = np.transpose(mclist)
	return (crhos <= mclistT).all(axis=1), (crhos > mclistT).all(axis=1)



#perfectMismatchCheckQuick: Check if there is a perfect mismatch condition, given matching values of all fields and codes, all resonance conditions of all codes, and vigilance parameters
#return index of perfect mismatch code and True/False (is perfect mismatch)-- Jan 2025
def perfectMismatchCheckQuick(mlist, allres, pmrhos):
	mconds = resonanceCheckQuick(mlist, pmrhos)
	if len(allres) <= 0:
		return -1, False
	pmlist = list(np.logical_and(mconds, np.invert(allres)))
	if any(pmlist):
		return pmlist.index(True), True
	else:
		return -1, False
...
#pmismatchXtd : extended pmismatch to check if given matching criteria mcriteria, it's in perfect mismatch,
#with additional conditions based on input vector (invect) and weight vector (wvect). The extra condition (enabled 
# with cccheck) looks at the magnitude of the input vector and weight vector as additional criteria for perfect mismatch 
#return True or False
def pmismatchXtd(match,mcriteria,invect=[],wvect=[],cccheck=False,midval=0.5):
    if cccheck:
        #if (match[0] >= 1.0) and ((sum(wvect)/len(wvect)) == (sum(invect)/len(invect))):
        if (match[0] >= 1.0) and (sum(wvect) == sum(invect)):
            return True
    else:
        #if np.all(match >= mcriteria):
        if match[0] >= 1.0:
            return True
    return False
	#basicpm = np.all(match >= mcriteria)
	#if basicpm and cccheck:
		#if sum(invect)/len(invect) > midval or ((sum(wvect)/len(wvect)) > (sum(invect)/len(invect))):
	#	if (sum(wvect)/len(wvect)) != (sum(invect)/len(invect)):
			#print(f'special condition in={sum(invect)} and weight={sum(wvect)} found, skipping perfect mismatch!!')
	#		return False
	#return basicpm
...
def refreshComplItemSchema(fschema,idx=-1):
	uschema = copy.deepcopy(fschema)
	if 'compl' in uschema:
		if uschema['compl']:
			if idx >= 0:
				if uschema['val'][idx] == -1:
					uschema['val'][idx] = 1
					uschema['vcompl'][idx] = 1
				elif uschema['val'][idx] == -2:
					uschema['val'][idx] = 0
					uschema['vcompl'][idx] = 0
				else:		
					uschema['vcompl'][idx] = 1 - uschema['val'][idx]
	return uschema
